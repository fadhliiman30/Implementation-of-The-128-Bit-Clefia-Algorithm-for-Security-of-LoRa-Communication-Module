...
void gfn_inverse(int rounds, uint32_t x[4], uint32_t round_keys[], uint32_t y[]) {
    uint32_t temp[4] = {x[0], x[1], x[2], x[3]};
    for (int i = 0; i < rounds; ++i) {
        temp[1] ^= f0(temp[0], round_keys[2 * (rounds - i) - 2]);
        temp[3] ^= f1(temp[2], round_keys[2 * (rounds - i) - 1]);
        uint32_t decoy = temp [3];
        temp[3] = temp[2];
        temp[2] = temp[1];
        temp[1] = temp[0];
        temp[0] = decoy;
...
void decrypt(uint32_t c[4], uint32_t wk[4], uint32_t rk[4], uint32_t* y) {
    uint32_t wk_add[4] = {c[0], c[1] ^ wk[2], c[2], c[3] ^ wk[3]};
    uint32_t temp_hasil[4];
    gfn_inverse(18, wk_add, rk, temp_hasil);
    y[0] = temp_hasil[0];
    y[1] = temp_hasil[1] ^ wk[0];
    y[2] = temp_hasil[2];
    y[3] = temp_hasil[3] ^ wk[1];
}
...
{ 
  ...
  uint32_t c[4] = {data.c0, data.c1, data.c2, data.c3};
  ...
  float latitude;
  uint32_t *ltd;
  ltd = (uint32_t*)&latitude;
  *ltd = msg[0];

  float longitude;
  uint32_t *longi;
  longi = (uint32_t*)&longitude;
  *longi = msg[1];
  ...
  Serial.print(data.c0, HEX);
  Serial.print(data.c1, HEX);
  Serial.print(data.c2, HEX);
  Serial.println(data.c3, HEX);
